...
BOOKING_URL = "<https://dubtraining.as.me/schedule/ad1bb86c/appointment/84360440/calendar/any?|Book Here!>"
...
def load_known_dates() -> set[str]:
    if STATE_FILE_ALL.exists():
        return set(json.loads(STATE_FILE_ALL.read_text()))
    return set()
...
def main() -> None:
    ...
    ### TEST ONLY ### save_known_dates(current_dates)
    ...
    print("filtered dates \n" + "\n".join(f"  - {d}" for d in sorted(filtered_dates)))
    ...
    print()
    filtered_dates_string = {d.strftime("%Y-%m-%d") for d in filtered_dates}
